...
data_filepath = './data/Kaggle.csv';

df = pd.read_csv(data_filepath, header=0, sep=';')

converted_records_leaked = pd.to_numeric(df['Records Lost'], errors='coerce')
df['Records Lost'] = converted_records_leaked
df = df[np.isfinite(df['Records Lost'])]

df['Method of Leak'].replace('', np.nan, inplace=True)
df.dropna(subset=['Method of Leak'], inplace=True)
...
def Add_Dash(server):
    ...
    dash_app = Dash(server=server,
                    ...
                    routes_pathname_prefix='/dashkaggle/')
    ...
    def display_value(value):
        if value == "Records lost by Entity":
            ...
            return go.Figure(data=[
                go.Pie(labels=df["Method of Leak"], values=df["Records Lost"], hole=0.2)
            ])
            ...
            fig_pie.update_traces(
                ...
                textfont_size=10,
                marker=dict(
                    colors=df["Sector"], line=dict(color='#000000', width=1.2)))
            return fig_pie
